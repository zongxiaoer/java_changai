<template>
  <div slot="right" class="content-page" v-if="uncompletedResearch_page">
    <div class="content">
      ...
      <div >
        ...
        <el-table
          ...
          >
            <template slot-scope="scope">
              <router-link :to="{ path: '/subjects/report4',query:{sid:scope.row.sid,quitLogId:scope.row.qiut_log_id,schemeId:scope.row.D2_id,flag:1}}">
                <el-button type="text" class="btnStyle" size="small"  v-if="scope.row.entry_status  == 2">录入</el-button>
              </router-link>
              <router-link :to="{ path: '/home/show',query:{id:scope.row.D2_id,sid:scope.row.sid}}">
...
<script>
  export default {
    ...
    data () {
      return {
        uncompletedResearch_page:false,
        btnAuth:{
    ...
    mounted(){
      let obj = this.checkPageAuth(this,"uncompletedResearch_page",this.btnAuth)
...
//      this.queryDepartMent()
    ...
    methods:{
      ...
      query(pageNo,pageSize){
        $axios_http({
          url:"/base/hospital/person/quitlog/query",
          data:{
            name:this.qc.name,
            sid:this.qc.sid,
            phone:this.qc.phone,
