    ...
    <div class="content">
      <div class="filter-container">
        ...
        <el-form :model="qc" :inline=true class="clear">
          ...
          <div>
            ...
            <!--v-if="btnAuth.subjectsList_add_btn">新增-->
            <!--</el-button>-->
      ...
      <div>
        <div class="table-btn-grooup">
          ...
          <!--<el-button  type="primary" size="small" icon="el-icon-close"-->
                     <!--v-if="btnAuth.subjectsList_EXCEL_btn" @click="openQuitDialog">退出研究-->
          <!--</el-button>-->
        </div>
        <el-dialog :visible.sync="quitDialog" :show-close="false" >
          <el-form :model="quitForms" :rules="quitFormsRules" ref="quitForms" >
            ...
            <div class="dialog-footer" style="text-align: center;">
              <el-button size="small" type="primary" @click="addQuitData('quitForms')" ref="addBtn" id="quitBtn">提交</el-button>
          ...
          >
            <template slot-scope="scope">
              <router-link :to="{path:'subjectsDetail',query:{id:scope.row.sid}}">
                <el-button type="text" size="small" v-if="btnAuth.subjectsList_EXCEL_btn">查看详情</el-button>
              </router-link>
              <el-button type="text" size="small" v-if="btnAuth.subjectsList_EXCEL_btn && (scope.row.overallStatusCy == 1 || scope.row.overallStatusCy == 3 || scope.row.overallStatusCy == 4)" @click="openQuitDialog(scope.row)">退出研究</el-button>
              <el-button type="text" size="small" v-if="btnAuth.subjectsList_EXCEL_btn && scope.row.overallStatusCy ==2 " @click='openInsertDialog(scope.row.sid)'>重新入组</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页栏-->
          ...
          if (valid) {
            $axios_http({
              ...
              this.$confirm('现在去填写违反方案记录表?', '提示', {
                ...
                this.$router.push({ path: '/subjects/report4',query:{sid:res.data.data[0].sid,quitLogId:res.data.data[0].quitLogId,schemeId:res.data.data[0].schemeId,flag:4}})
              }).catch(() => {
...
//        $axios_http({
//          method:'get',
//          url: "/base/hospital/person/queryExcel",
//          vueObj: this
//        }).then((res) => {
//          $successMsg(this, "导出成功")
//        })
      },
      //删除一条数据
      del(id){
