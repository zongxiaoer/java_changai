<template>
  <div class="j_content" v-if="showReport3_page">
     ...
     <div id="print">
      <el-form :inline="true" :model="formInline" class="demo-form-inline" label-width="170px" :rules="rules" ref="formInline" :disabled="show" >
        ...
        <table class="j_table cmc">
          ...
          <p class="texts" style="line-height:2;">
      ...
      <div class="text-center" style="margin-top: 40px;" >
        <approvalDialog ref="approvalDialog" @refreshList="queryDate" :id="ShowData.id" :approvalArr="approvalArr" v-if="$store.state.hospitalType==3 && ShowData.approvalStatus==0"></approvalDialog>
    ...
    data() {
      ...
      var item_a_s_6_other = (rule, value, callback) => {
        if(this.formInline.item_a_s_6){
          ...
          if (value !== '') { this.formInline.item_a_s_6 = true;}
          callback()
        }
      };
...
//item_c_type
      ...
      return {
        approvalArr:{
          formType:"HOSPITAL_SCREENING_NOTIFICATION",
        },  //解除锁定相关信息
        ShowData:null,
        showReport3_page:false,
        show:'',
        btnAuth:{},
        formInline: {
          name: '张三',
    ...
    // mounted(){
    //   let obj = this.checkPageAuth(this,"showReport3_page",this.btnAuth);
...
//            return;
            $axios_http({
              url:"/base/screening/notification/addScreeningNotification",
              data:dd,
              vueObj:this
            }).then((res)=> {
              //显示操作成功浮动提示框
