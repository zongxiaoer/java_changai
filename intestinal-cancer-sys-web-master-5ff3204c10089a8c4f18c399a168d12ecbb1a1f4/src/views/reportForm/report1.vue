<template>
  <div class="j_content" v-if="area_colonoscopy_add_result_page">
    <div style="min-width: 941px;">
        ...
        <div class="tables" style="position:relative;">
          ...
          <el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline" :rules="rules2" :disabled="show">
            <el-form-item label="参与者是否进行了直肠指诊（无法耐受肠镜检查者）？" class="formoneline" prop="item_2_1">
              <el-radio-group v-model="formInline.item_2_1" @change='changeRadio'>
                <el-radio :label="1">是</el-radio>
                <el-radio :label="2">否</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="有无肿块" class="formoneline" prop="item_2_1_a" v-if="item_2_1_a_show || formInline.item_2_1=='1'">
            ...
            <el-form-item label="参与者是否完成了结肠镜检查？" class="formoneline" prop="item_2_2">
              <el-radio-group v-model="formInline.item_2_2"  :disabled="show || $route.query.flag=='edit'">
                <el-radio :label="1">是</el-radio>
                <el-radio :label="2">否</el-radio>
              </el-radio-group>
            </el-form-item>
        ...
        <div class="tables">
          <el-form :inline="true" :model="formInline2" ref="formvalidate3" class="demo-form-inline" :rules="formvalidate3rules" :disabled="show">
            ...
            <el-form-item label="是否检出息肉？" class="formoneline" prop="item_3_7">
              <el-radio-group v-model="formInline2.item_3_7"  :disabled="show" @change="checkRecordList">
                <el-radio :label="1">是</el-radio>
                <el-radio :label="2">否</el-radio>
              </el-radio-group>
        ...
        <div class="tables">
          <div v-for="(x,index) in lesionsRecordList">
            ...
            <div style="border:1px solid #999999;padding:20px; margin-bottom: 20px;">
              <!-- 必填 -->
              <el-form :inline="true" :model="x" :validate-on-rule-change="false" class="demo-form-inline" ref="lesionsRecordForm" :rules="lesionsRecordRule" :disabled="show" v-if="recordListShow">
              ...
              <!-- 非必填 -->
              <el-form :inline="true" :model="x" :validate-on-rule-change="false" class="demo-form-inline" ref="lesionsRecordForm" :rules="lesionsRecordFalseRule" :disabled="show" v-if="!recordListShow">
                <el-form-item label="部位:" class="formhalfline" prop="item1">
                  <el-select v-model="x.item1" placeholder="请选择" size="small"  :disabled="show">
                    <el-option
                      v-for="item in partmentoptions"
          ...
          <uploadMultipleSubmit :uploadFileList="fileList" v-if="!show" @getUrl='getUrl' @getUrl2='getUrl2'></uploadMultipleSubmit>
          <!-- 查看图片展示区 -->
      ...
      <div class="text-center" style="margin-top: 40px;" v-if="$route.query.flag!='edit'">
        <!-- 编辑相关btn -->
        ...
        <router-link :to="{path:'/colonscopy/report1',query:{id:$route.query.id,sid:$route.query.sid,colonoscopyRecordId:$route.query.colonoscopyRecordId,flag:'edit'}}" v-if="$store.state.hospitalType==2 && ShowData.editStatus==1 && $route.query.overallStatusCy!=2 && $route.query.overallStatusCy!=4">
          <el-button type="primary">编辑</el-button>
        </router-link>
        <!-- 编辑相关btn -->
        <router-link :to="{path:'/colonscopy/report2',query:{id:ShowData.pathologyId,sid:ShowData.sid,overallStatusCy:$route.query.overallStatusCy,show:1}}">
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        area_colonoscopy_add_result_page:false,
        recordListShow:true,
        btnAuth:{},
        fileList:[],
        users:{
        ...
        formInline: {
          ...
          item_2_1_a: null,//int 有无肿块 1：有，2：无
    ...
    created() {
      let obj = this.checkPageAuth(this,"area_colonoscopy_add_result_page",this.btnAuth)
      this.formInline.sid = this.$route.query.sid;
      this.formInline.id = this.$route.query.id
      this.show = this.$route.query.show
      this.queryDetail()
      if(this.show || this.$route.query.flag=='edit'){
        this.queryInfo()
        // this.rules2 = {}
    ...
    methods:{
      ...
      checkRecordList(){
        if(this.formInline2.item_3_7=="1"){
      ...
      changeRadio(){
        if(this.formInline.item_2_1==1){
          this.item_2_1_a_show=true
        }else{
          this.item_2_1_a_show=false
          this.formInline.item_2_1_a=null
        }
      },
      //查询user
      queryDetail(){
      ...
      clearFiled(type){ // 清空表单 type为空时清空所有
        ...
        this.formInline6.pathology = null
      },
      submit(){ //提交
        //按钮权限判断
        ...
        if(validd){
          ...
          if(dd.item_2_1_a == null){
            delete dd.item_2_1_a
          }
          ...
          if(that.formInline.item_2_2 == '1'){
            ...
            dd.lesionsRecordList.filter(item=>{
              for(let key in item){
                if(!item[key]){
                  item[key]=null
                }
          ...
          if(this.uploadUrls2.length>0){
            dd.fileUploads=this.uploadUrls2;
          }else{
            dd.fileUploads=this.uploadUrls;
          }
          // 新增，编辑的接口地址
          let _url = null;
          if(this.$route.query.flag == 'edit'){
            _url = "/base/colonoscopy/result/updateColonoscopyResult"
            dd.id = this.$route.query.id
            dd.colonoscopyRecordId = this.$route.query.colonoscopyRecordId
            dd.fileUploads=objDeepCopy(this.fileList);
            // dd.fileUploads=this.fileList;
            dd.fileUploads && dd.fileUploads.filter( item => {
              item.fileUploadTime=this.dateFormater(item.fileUploadTime)
              item.status=null
              if(item.filePath.indexOf('?filePath=')>-1){
                item.filePath=item.filePath.split('?filePath=')[1]
              }
            })
          }else{
            _url = "/base/colonoscopy/result/addColonoscopyResult"
          }
          // 保存数据
           if(this.$route.query.flag=='edit'){
            /**
      ...
      saveColonResult(dataObj,dataUrl){
        $axios_http({
            ...
            // 新增才去填病理
            if(this.$route.query.flag!='edit' && this.formInline6.pathology=="1"){
              this.$confirm('<p>现在去填写《表B2-病理结果记录表》吗？</p>', '', {
                       ...
                       this.$router.push({path:'/colonscopy/report2',query:{sid:this.$route.query.sid,id:this.$route.query.id,resultId:this.$route.query.resultId,flag:this.$route.query.flag}});
                        //清空数据
                      //  this.clearFiled()
                      }).catch(() => {
                         this.$router.go(-1);
      ...
      applyEdit () {   //申请编辑弹窗
        ...
        this.applyArr.formType="HOSPITAL_COLONOSCOPY_RESULT";
            ...
            if(res.data.data){
              if(res.data.data.notification_entry_status==2){
                if(res.data.data.notification_issue_status==2) {
                  ...
                  // 告知书已录入
                    this.$confirm('<p>肠镜结果已变更，是否重新录入告知书？</p>', '', {
                          confirmButtonText: '是',
                          cancelButtonText: '否',
                          dangerouslyUseHTMLString: true,
                          showClose:false,
